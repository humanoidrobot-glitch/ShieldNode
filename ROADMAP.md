...
- [x] **Decentralize challenge-response** — Replaced trusted challenger multisig with open challenge bonds. Anyone can challenge a node by posting a 0.01 ETH bond. Bond returned on node response (honest node proved). Bond returned + slash evidence on expiration (node failed). Per-challenger-per-node cooldown (6 hours) prevents griefing. `canChallenge()` view for pre-flight checks. 17 tests covering bond deposit, return on response, return on expiration, insufficient bond, cooldown enforcement, multi-challenger independence
...
- [x] **ZK node eligibility proofs** — Commitment-based registry where nodes prove they meet selection criteria (stake, uptime, no slashes) without revealing which node they are. Hardens against enumeration attacks by state actors
- [x] **ZK no-log compliance proofs** — Nodes periodically prove their runtime state contains no connection metadata beyond active sessions. ZK guest (mode 2) verifies 6 structural invariants: session count bounded, no zero-key sessions, registry consistency, no orphaned bandwidth entries, bandwidth consistency, no duplicate session IDs. Node serializes a `ComplianceSnapshot` from `RelayService` + `BandwidthTracker`. On-chain `NoLogVerifier.sol` tracks per-node proof freshness (6-hour window), last state hash, session count. Limitation: proves declared state only — OS-level capture requires TEE (Phase 5)
- [x] **ZK proof of honest relay (research)** — Evaluated RISC Zero v5 for proving correct packet forwarding. Result: **feasible today**. The relay function (ChaCha20-Poly1305 decrypt + routing extraction + SHA-256 hashing) compiles to 131K zkVM cycles. Benchmarked: 833ms proof generation on GPU (RTX 2080 Ti), 17.6s on CPU, 9ms receipt verification. Per-packet proving is infeasible at line rate, but random sampling (prove N packets from a challenge window) is practical — a node can respond to a challenge in <1s with GPU. Full implementation: guest program (`zkvm/guest/`), host with execute/prove modes (`zkvm/host/`), methods crate for ELF embedding, Docker environments for CPU and CUDA GPU proving. On-chain verifiers: `RelayProofVerifier.sol` and `ExecutionTraceVerifier.sol`. See `docs/anti-logging-research.md` §8 for threat model analysis
- [x] **PQ Sphinx packet format** — Each packet carries per-hop hybrid KEM ciphertexts (X25519 + ML-KEM-768) so hops derive layer keys from the packet itself, independent of session handshake. Recursive nesting: each peel reveals the next hop's complete packet (inner headers encrypted). Version byte 0xFF for wire-level dispatch. Per-session KEM amortization via PqSessionKeys (encapsulate once, reuse ciphertext for all packets). Overhead: ~1,189 bytes per hop header (3.6 KB for 3-hop), handled by existing packet normalization/fragmentation. Both node and client implementations with 10 tests (1/3-hop roundtrip, serialization, caching, tampering, version dispatch)
...
| Sphinx DH operations | X25519 | Hybrid X25519 + ML-KEM-768 per-hop KEM | 6 (done) |
