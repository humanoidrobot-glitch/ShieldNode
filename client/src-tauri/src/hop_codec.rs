...
///
/// The endpoint's own port is ignored — the relay port is used instead
/// because relay traffic goes to a separate UDP port (typically WG port + 1).
...
mod tests {
    ...
    #[test]
    fn endpoint_to_next_hop_bad_input() {
        assert!(endpoint_to_next_hop("not-an-addr", 51821).is_err());
    }
}
