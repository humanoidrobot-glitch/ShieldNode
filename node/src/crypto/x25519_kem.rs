...
/// x25519-dalek's default features include `zeroize`, which derives
/// `Zeroize + ZeroizeOnDrop` on `StaticSecret`. The inner [u8; 32] is
/// automatically zeroed via volatile writes when the key is dropped.
